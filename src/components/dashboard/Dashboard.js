...
import { getContrastColor, isLowContrast } from '../../utils/colorContrast';
import ThemeConfigDialog from "./ThemeConfigDialog";
import PaletteIcon from '@mui/icons-material/Palette';
import FontColorConfigDialog from './FontColorConfigDialog';
...
export default function Dashboard() {
  ...
  const [fontColors, setFontColors] = useState(() => {
    return JSON.parse(localStorage.getItem("fontColors") || '{"primary":"#222","secondary":"#666","text":"#222","disabled":"#aaa"}');
  });
      ...
      <Box>
        <Box display="flex" flexDirection="column" alignItems="center" mb={2} gap={1}>
          <Box display="flex" alignItems="center" gap={1}>
            <IconButton onClick={() => setConfigOpen(true)} sx={{color: getContrastColor(customTheme.palette.background.default)}}>
              <SettingsIcon />
            </IconButton>
            <Typography
              variant="h5"
              fontWeight="bold"
              sx={{ mr: 1,
                color: isLowContrast(customTheme.palette.background.default, fontColors.primary)
                  ? getContrastColor(customTheme.palette.background.default)
                  : fontColors.primary
              }}
            >
              {themeConfig.name}
            </Typography>
            <IconButton onClick={() => setThemeOpen(true)} title="Personalizar colores y nombre" sx={{color: getContrastColor(customTheme.palette.background.default)}}>
              <PaletteIcon />
            </IconButton>
            <IconButton onClick={() => setFontColorOpen(true)} title="Colores de letras" sx={{color: getContrastColor(customTheme.palette.background.default)}}>
              <FormatColorTextIcon />
            </IconButton>
            {/* Fuerza alto contraste en textos secundarios del header */}
            <Typography variant="subtitle2" sx={{ color: getContrastColor(customTheme.palette.background.default), ml: 1 }}>
              Ajusta los colores para mejor visibilidad
            </Typography>
          ...
          <Box display="flex" gap={2} alignItems="center">
            <TextField
              ...
              sx={{
                '& .MuiInputLabel-root': {
                  color: getContrastColor(customTheme.palette.background.default)
                },
                '& .MuiInputBase-input': {
                  color: getContrastColor(customTheme.palette.background.default)
                }
        ...
        <FontColorConfigDialog open={fontColorOpen} onClose={() => setFontColorOpen(false)} onSave={(colors) => {
          setFontColors(colors);
          localStorage.setItem("fontColors", JSON.stringify(colors));
          setFontColorOpen(false);
        }} initialColors={fontColors} />
        {/* Mensaje de ayuda si el contraste es bajo */}
        {isLowContrast(customTheme.palette.background.default, fontColors.primary) && (
          <Typography variant="caption" sx={{ color: '#d32f2f', mt: 1, mb: 1 }}>
            ¡Advertencia! El color de texto principal tiene bajo contraste con el fondo.
          </Typography>
        )}
      </Box>
    </ThemeProvider>
